...
(setq control 0)
(setq vectorx 0) ;vector için hangi sıraya ekleme yapacağımı tutar(*my-2d-vector*)
(defvar *my-2d-vector* (make-array '(1000 2) :element-type t :initial-element "ai")) ;değişkenler için
(defvar *func-vector* (make-array '(1000 2) :element-type t :initial-element "ai")) ;fonksiyonlar için
(setq vectory 0) ;vector için hangi sıraya ekleme yapacağımı tutar(*func-vector*)
(defvar float-list '()) ;işlem yapmam gereken kısımlarda o sayıları yada değişken isimlerini tutup ona göre işlem yapıyorum
(defvar int-list '()) ;girilen tüm elemanlara bir sayı belirledim ve her kelime hangi manaya geliyor bu sayıya göre tutuyorum
...
(defun split-string-for-b (input-string)
  ;; Giriş değeri bir string mi kontrol et
  (if (stringp input-string)
      ;; Giriş değeri bir string ise devam et
      (let ((b-position (position #\b input-string)))
        ;; 'b' karakterinin pozisyonunu bul
        (if b-position
            ;; Eğer 'b' karakteri bulunursa devam et
            (let* ((before-b (subseq input-string 0 b-position))
                   (after-b (subseq input-string (1+ b-position))))
              ;; 'b' karakterinden önceki ve sonraki kısımları içeren bir liste oluştur
              (list before-b after-b))
            ;; 'b' karakteri bulunamazsa hata mesajı gönder
            (write-line "String does not contain 'b'")))
      ;; Giriş değeri bir string değilse hata mesajı gönder
      (write-line "Invalid input: not a string")))
    ...
    (let ((count 0))
        (if (string= "and" str)
            (progn
                ...
                ;(write-line "KW_AND")
                (push 2 int-list)
                ;(return-from keyword-control nil)
                (incf count)
            )
        )
        (if (string= "or" str)
...
; bir string listinin son stringinin son karakteri bir parantez mi diye kontrol eder.
(defun is-last-char-parenthesis-p (words)
  (and (stringp (first (last words))) ; Son eleman bir string mi?
       (string/= (first (last words)) "") ; String boş değil mi?
       (char= (char (first (last words)) (1- (length (first (last words))))) #\)) ; Son karakter bir kapalı parantez mi?
       ))

; bir string listinin ilk karakteri parantez mi diye kontrol eder.
(defun is-first-char-open-parenthesis-p (words)
  (and (listp words) ; Liste mi?
       (not (null words)) ; Boş değil mi?
       (stringp (first words)) ; İlk eleman bir string mi?
       (not (string= (first words) "")) ; String boş değil mi?
       (char= (char (first words) 0) #\() ; İlk karakter açık parantez mi?
       ))
            ...
            (if (< b-count 2) ;eğer string içinde 1 den fazla b harfi varsa VALUEF yazdırıyorum.
                (if (> b-count 0)
                    (progn
                        ;(print str)
                        ;(write-line "VALUEF")
                        (push (parse-valuef str) float-list)
                        (push 26 int-list)
                    )
                )
            )
...
;bir string alıp valuef ise float bir sayıya çeviren fonksiyondur
(defun parse-valuef (input-string)
  (if (stringp input-string)
      (let* ((parts (split-string-for-b input-string))
...
;vector içersinde belirli bir stringi arayıp varsa indeksini return eder.
(defun find-string-in-vector (vector target-string)
    (loop for i from 0 below 1000
        until (string= target-string (aref vector i 0))
        finally (return (if (< i (array-dimension vector 0))
                            i
                            nil))
    )
)

(defun print-fraction (num)
  (multiple-value-bind (numerator denominator) (truncate num)
    (format t "~a b ~a~%" numerator denominator)))
...
(defun whole-operations (words)
    (let (paranthesList (list)) ;bir word içinde parantez varsa parçalanan halini list olarak tutar.
        (if (and (is-first-char-open-parenthesis-p words) (is-last-char-parenthesis-p words))
            (progn
                ...
                (if (= (length int-list) 5)
                    (progn
                        (if (= (nth 3 int-list) 20)
                            (progn
                                (if (= (nth 1 int-list) 26)
                                    (if (= (nth 2 int-list) 26)
                                        (progn
                                            (let ((result (+ (nth (- (length float-list) 2) float-list)
                                                             (nth (- (length float-list) 1) float-list))))
                                                (format t "~ab~a~%" (numerator result) (denominator result))
                                            )
                                        )
                                        (progn
                                            ;(format t "Float List: ~,2F~%" int-list)
                                            (write-line "Syntax Error..!")
                                        )
                                    )
                                    (progn
                                        ;(format t "Float List: ~a~%" int-list)
                                        (write-line "Syntax Error..!")
                                    )
                                )
                            )
                        )
                        (if (= (nth 3 int-list) 21)
                            (progn
                                (if (= (nth 1 int-list) 26)
                                    (if (= (nth 2 int-list) 26)
                                        (progn
                                            (let ((result (- (nth (- (length float-list) 1) float-list)
                                                             (nth (- (length float-list) 2) float-list))))
                                                (format t "~ab~a~%" (numerator result) (denominator result))
                                            )
                                        )
                                        (progn
                        ...
                        (if (= (nth 3 int-list) 2)
                            (progn
                                (if (or (= (nth 1 int-list) 18) (= (nth 1 int-list) 19))
                                    (if (or (= (nth 2 int-list) 18) (= (nth 2 int-list) 19))
                                        (progn
                                            (let ((result (and (nth (- (length float-list) 1) float-list)
                                                             (nth (- (length float-list) 2) float-list))))
                                                    (if result
                                                        (write-line "true")
                                                        (write-line "false")
                        ...
                        (if (= (nth 3 int-list) 3)
                            (progn
                                (if (or (= (nth 1 int-list) 18) (= (nth 1 int-list) 19))
                                    (if (or (= (nth 2 int-list) 18) (= (nth 2 int-list) 19))
                                        (progn
                                            (let ((result (or (nth (- (length float-list) 1) float-list)
                                                             (nth (- (length float-list) 2) float-list))))
                                                    (if result
                                                        (write-line "true")
                                                        (write-line "false")
                        ...
                        (if (= (nth 3 int-list) 5)
                            (progn
                                (if (= (nth 1 int-list) 26)
                                    (if (= (nth 2 int-list) 26)
                                        (progn
                                            (let ((result (equal (nth (- (length float-list) 2) float-list)
                                                             (nth (- (length float-list) 1) float-list))))
                                                    (if result
                                                        (write-line "true")
                                                        (write-line "false")
                        ...
                        (if (= (nth 3 int-list) 11)
                            (progn
                                (if (= (nth 2 int-list) 25)
                                    (if(or (= (nth 1 int-list) 26)
                                    (= (nth 1 int-list) 18)
                                    (= (nth 1 int-list) 19)
                                    (= (nth 1 int-list) 24))
                                        (progn
                                            (setf (aref *my-2d-vector* vectorx 0) (nth (- (length float-list) 1) float-list))
                                            (setf (aref *my-2d-vector* vectorx 1) (nth (- (length float-list) 2) float-list))
                                            (incf vectorx 1)
                                        )
                                        (write-line "Syntax Error!!!")
                                    )
                ...
                (if (= (length int-list) 4)
                    (progn
                        (if (= (nth 2 int-list) 4)
                            (progn
                                (if (or (= (nth 1 int-list) 18) (= (nth 1 int-list) 19))
                                    (progn
                                        (let ((result (not (nth (- (length float-list) 1) float-list))))
                                                (if result
                                                    (write-line "true")
                                                    (write-line "false")
                                                )
                        ...
                        (if (= (nth 2 int-list) 17)
                            (progn
                                (if (= (nth 1 int-list) 25)
                                    (progn
                                        (let* ((target-string (nth (- (length float-list) 1) float-list))
                                              (index (find-string-in-vector *my-2d-vector* target-string)))
                                            (if index
                                                (if (or (eql (aref *my-2d-vector* index 1) t) (eql (aref *my-2d-vector* index 1) nil))
                                                    (if (aref *my-2d-vector* index 1)
                                                        (write-line "true")
                                                        (write-line "false")
                                                    )
                                                    (format t "~ab~a~%" (numerator (aref *my-2d-vector* index 1)) (denominator (aref *my-2d-vector* index 1)))
                                                )
                                                (write-line "Syntax Error. This variable does not exist!")
                                            )
                                        )
                                    )
                                    (progn
                        ...
                        (if (not (or (= (nth 2 int-list) 17)
                            (= (nth 2 int-list) 4)))
                            (write-line "Syntax Error..!")
                        )
                    )
                )
                (if (= (length int-list) 3)
                    (progn
                        (if (= (nth 1 int-list) 25)
                            (progn
                                (let* ((target-string (nth (- (length float-list) 1) float-list))
                                      (index (find-string-in-vector *func-vector* target-string)))
                                    (if index
                                        (if (or (eql (aref *func-vector* index 1) t) (eql (aref *func-vector* index 1) nil))
                                            (if (aref *func-vector* index 1)
                                                (write-line "true")
                                                (write-line "false")
                                            )
                                            (format t "~ab~a~%" (numerator (aref *func-vector* index 1)) (denominator (aref *func-vector* index 1)))
                                        )
                                        (write-line "Syntax Error. This function does not exist!")
                                    )
                                )
                            )
                        )
                ...
                (if (= (length int-list) 9)
                    (progn
                        (if (= (nth 7 int-list) 12)
                            (progn
                                (if(or (= (nth 4 int-list) 20)
                                (= (nth 4 int-list) 21)
                                (= (nth 4 int-list) 22)
                                (= (nth 4 int-list) 23)
                                (= (nth 4 int-list) 2)
                                (= (nth 4 int-list) 3))
                                    (progn
                                        (if (= (nth 4 int-list) 20)
                                            (progn
                                                (if (and (= (nth 3 int-list) 26)
                                                (= (nth 2 int-list) 26))
                                                    (progn
                                                        (write-line "#function")
                                                        (let ((result (+ (nth (- (length float-list) 2) float-list)
                                                            (nth (- (length float-list) 3) float-list))))
                                                            (setf (aref *func-vector* vectory 0) (nth (- (length float-list) 1) float-list))
                                                            (setf (aref *func-vector* vectory 1) result)
                                                            (incf vectory 1)
                                                        )
                                                    )
                                                )
                                                (if (and (= (nth 3 int-list) 25)
                                                (= (nth 2 int-list) 25))
                                                    (progn
                                                        (let* ((target-string (nth (- (length float-list) 3) float-list))
                                                              (index (find-string-in-vector *my-2d-vector* target-string)))
                                                            (if index
                                                                (progn
                                                                    (pop float-list)
                                                                    (let* ((target-string (nth (- (length float-list) 2) float-list))
                                                                          (index2 (find-string-in-vector *my-2d-vector* target-string)))
                                                                        (if index2
                                                                            (progn
                                                                                (pop float-list)
                                                                                (push (aref *my-2d-vector* index2 1) float-list)
                                                                                (push (aref *my-2d-vector* index 1) float-list)
                                                                                (write-line "#function")
                                                                                (let ((result (+ (nth (- (length float-list) 2) float-list)
                                                                                      (nth (- (length float-list) 3) float-list))))
                                                                                    (setf (aref *func-vector* vectory 0) (nth (- (length float-list) 1) float-list))
                                                ...
                                                (if (not (or (and (= (nth 3 int-list) 26)
                                                    (= (nth 2 int-list) 26))
                                                    (and (= (nth 2 int-list) 25)
                                                    (= (nth 2 int-list) 25))))
                                                    (write-line "Syntax Error..")
                                                )
                                            )
                                        )
                                        (if (= (nth 4 int-list) 21)
                                            (progn
                                                (if (and (= (nth 3 int-list) 26)
                                                    (= (nth 2 int-list) 26))
                                                    (progn
                                        ...
                                        (if (= (nth 4 int-list) 2)
                                            (if (and (or (= (nth 3 int-list) 18) (= (nth 3 int-list) 19))
                                            (or (= (nth 2 int-list) 18) (= (nth 2 int-list) 19)))
                                                (progn
                                                    (write-line "#function")
                                                    (let ((result (and (nth (- (length float-list) 2) float-list)
                                                        (nth (- (length float-list) 3) float-list))))
                                                        (setf (aref *func-vector* vectory 0) (nth (- (length float-list) 1) float-list))
                                                        (setf (aref *func-vector* vectory 1) result)
                                                        (incf vectory 1)
                                        ...
                                        (if (= (nth 4 int-list) 3)
                                            ...
                                            (or (= (nth 2 int-list) 18) (= (nth 2 int-list) 19)))
                                                (progn
                                                    (write-line "#function")
                                                    (let ((result (or (nth (- (length float-list) 2) float-list)
                                                        (nth (- (length float-list) 3) float-list))))
                                                        (setf (aref *func-vector* vectory 0) (nth (- (length float-list) 1) float-list))
                                                        (setf (aref *func-vector* vectory 1) result)
                                                        (incf vectory 1)
...
(write-line "--------------------------------------------------")
(write-line "")
(write-line "Welcome to Concrete Analysis")
(write-line "")
(write-line "--------------------------------------------------")
(write-line "")
(write-line "")
